		...
		do kk = -1, 1
		   do jj = -1, 1
			 ...
			 if(volfracborder(i+ii,j+jj,k+kk,1).ne.0.0d0)then
			   ...
			   !sigma_kappa = sigma_kappa + Sborder(i+ii,j+jj,k+kk,1)
			 endif
		      end do
		   end do
		end do
		...
		sigma_kappa = sigma_kappa !+ volfrac(i,j,k)			
	      ...
	      do compno = 1, 7
			 ...
			 if(volfracborder(i+ii,j+jj,k+kk,1).ne.0.0d0)then
                            ...
                            !volwgtavg = volwgtavg + Sborder(i+ii,j+jj,k+kk,1)*Sborder(i+ii,j+jj,k+kk,compno)
  ...
  integer :: i, j, k, compno
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
		...
		!stop
      	     do compno = 1, 7
	   	Sborder(i,j,k,compno) =  Sborder(i,j,k,compno)
	   	!Sborder(i,j,k,compno) =  (Sborder(i,j,k,compno)*volfracborder(i,j,k,1) - cons_adjust(i,j,k,compno))/volfracborder(i,j,k,1)
