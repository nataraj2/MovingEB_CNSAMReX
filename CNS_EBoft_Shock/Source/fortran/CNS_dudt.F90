...
module cns_dudt_module
  ...
  public:: cns_compute_dudt, cns_eb_compute_dudt, zerooutsolidwalls, redistributecovered
...
contains
  ...
  subroutine cns_eb_compute_dudt (lo,hi, dudt, utlo, uthi, &
    ...
    real(rt), intent(in) :: dx(3), prob_lo(3), dt
    integer, intent(in) :: level
    ...
    call compute_eb_divop(lo,hi,5,dx,prob_lo,dt,fhx,lfxlo,lfxhi,fhy,lfylo,lfyhi,fhz,lfzlo,lfzhi,&
         fx, fxlo, fxhi, fy, fylo, fyhi, fz, fzlo, fzhi, &
  ...
  do k = lo(3), hi(3)
     do j = lo(2), hi(2)
		...
		do compno=1,14
	    	   if(abs(u(i,j,k,compno)).le.1e-13)then
			u(i,j,k,compno)=0.0d0
		   endif
		enddo
	end do
    end do
  ...
  subroutine redistributecovered(level, lo, hi, u, ulo, uhi, volfrac, vlo, vhi)bind(c,name='redistributecovered')
  !subroutine redistributecovered(level, lo, hi, u, ulo, uhi)bind(c,name='redistributecovered')
  !subroutine redistributecovered(level, lo, hi, volfrac, vlo, vhi)bind(c,name='redistributecovered')

  implicit none
  integer, intent(in) :: level
  integer, dimension(3), intent(in) :: lo, hi, ulo, uhi, vlo, vhi
  !integer, dimension(3), intent(in) :: lo, hi, vlo, vhi
  real(rt), intent(in) :: u ( ulo(1): uhi(1), ulo(2): uhi(2), ulo(3): uhi(3),1)
  real(rt), intent(in) :: volfrac(vlo(1):vhi(1),vlo(2):vhi(2),vlo(3):vhi(3))
  integer :: i, j, k
  ...
  do k = lo(3)-2, hi(3)+2
     do j = lo(2)-2, hi(2)+2
        do i = lo(1)-2, hi(1)+2
		if(level.eq.2)then
		if(volfrac(i,j,k).ne.u(i,j,k,1))then
        	print*,i,j,k,volfrac(i,j,k), u(i,j,k,1)!, u(i,j,k,8)
		endif
        	!print*, u(i,j,k,1)
		endif
	end do
  ...
  end subroutine redistributecovered
