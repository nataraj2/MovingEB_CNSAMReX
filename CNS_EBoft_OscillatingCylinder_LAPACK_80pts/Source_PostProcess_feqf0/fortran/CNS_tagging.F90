...
contains
  ...
  subroutine cns_tag_denerror (lo, hi, tag, tlo, thi, rho, rlo, rhi, flag, flo, fhi, &
       dengrad, tagval, clearval, dx, time) bind(c,name='cns_tag_denerror')
    ...
    real(amrex_real), intent(in) :: dx(3), time
    ...
    do       k = lo(3), hi(3)
       ...
       do    j = lo(2), hi(2)
		...
		wx=abs(rho(i+1,j,k)-2.0d0*rho(i,j,k)+rho(i-1,j,k))
                ...
                if ( z .ge. 2.0d0+50.0d0*time-3.0 .and. z .le.2.0d0+50.0d0*time+3.0d0 .and. abs(y).le.0.25d0) then
                !if ( z .ge. 10.0d0 .and. z .le. 12.0d0 .and. abs(y).le.0.25d0) then
                tag(i,j,k) = tagval
		else
		tag(i,j,k) = clearval
                endif
