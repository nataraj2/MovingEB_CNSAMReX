...
contains
  ...
  subroutine compute_diff_wallflux (divw, dxinv, i,j,k, &
    ...
    real(rt) :: weight
    real(rt) ::  ATA(matsize,matsize), ATb(matsize,nvar), dxi, dyi, dzi, vec(matsize), ebvel(nvar)
    ...
    ublade = 0.0d0
    vblade = -0.2d0*0.4d0*2.0d0*3.14159265359d0*24.375d0*sin(2.0d0*3.14159265359d0*24.375d0*time)
    wblade = 0.0d0
    ...
    x0 = -0.3125d0 + (i+0.5d0)*dx(1) + bct(1)*dx(1)
    ...
    if(abs(x-1.0d0/(dxinv(1)*2.0d0)).le.1e-5.and.level .eq. 4 .and. RKstep .eq. 2)then
