...
contains
  ...
  subroutine compute_diff_wallflux (divw, dxinv, i,j,k, &
    ...
    integer, parameter :: cluster_size = 3, matsize = 3, nvar = 3
    real(rt) :: weight
    ...
    ublade = 0.0d0
    vblade = -0.2d0*0.4d0*2.0d0*3.14159265359d0*24.375d0*0.8d0*sin(2.0d0*3.14159265359d0*24.375d0*0.8d0*time)
    wblade = 0.0d0
    ...
    do kk = k-cluster_size, k+cluster_size 
       do jj = j-cluster_size, j+cluster_size
		...
		if(ii.eq.i.and.jj.eq.j.and.kk.eq.k)then
			...
			do iind = 1, matsize
			   do jind = 1, matsize
				ATA(iind,jind) = ATA(iind,jind) + weight*vec(iind)*vec(jind)
			...
			do var = 1, nvar
				do iind = 1, matsize
					ATb(iind,var) = ATb(iind,var) + weight*(q(ii,jj,kk,var+1)-ebvel(var))*vec(iind)
				end do
			end do 
