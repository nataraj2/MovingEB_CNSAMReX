                ...
                const int max_coarsening_level,Real time)
{
	...
	if( myfile.eof() ) break;
	//std::cout << p1 << " "<< p2 << "\n";
	p = amrex::RealVect(AMREX_D_DECL(p1-0.25,p2,0.0));
        pts.push_back(p);
	}
	...
	pts.clear();
	//auto airfoil_extrude = EB2::extrude(airfoil,2);9
	auto airfoil_rotate = EB2::rotate(airfoil,2.00*3.14159265359/180.0*sin(2.0*3.14159265359*17.0077590009*time),2);
	//auto airfoil_rotate = EB2::rotate(airfoil,0.0*3.1415/180.0,2);
	//auto airfoil_translate = EB2::translate(airfoil_extrude,{-0.1,-0.1,0.0});

        auto gshop = EB2::makeShop(airfoil_rotate);
