...
contains
  ...
  subroutine cns_tag_denerror (lo, hi, tag, tlo, thi, rho, rlo, rhi, flag, flo, fhi, &
    ...
    real(kind=amrex_real)	      :: wx, wy, wz, mean, wtilde
    character(kind=c_char), intent(in) :: tagval, clearval

    integer :: i,j,k
    real(amrex_real) :: ax, ay, az, x, y
    ...
    do       k = lo(3), hi(3)
       do    j = lo(2), hi(2)
		...
		wz=abs(rho(i,j,k+1)-2.0d0*rho(i,j,k)+rho(i,j,k-1))
                mean=dsqrt((rho(i,j,k+1)+2.0d0*rho(i,j,k)+rho(i,j,k-1)**2+&
			    (rho(i,j+1,k)+2.0d0*rho(i,j,k)+rho(i,j-1,k)**2))+&
			    (rho(i+1,j,k)+2.0d0*rho(i,j,k)+rho(i-1,j,k)**2))
                wtilde=dsqrt(wx**2+wy**2+wz**2)/(mean+1e-10)
                if ( x.ge.-0.3d0 .and. x .le. 2.0d0 .and. abs(y) .le. 0.25d0) then
                tag(i,j,k) = tagval
                endif
	     endif
          end do
