...
module cns_dudt_module
  ...
  public:: cns_compute_dudt, cns_eb_compute_dudt, zerooutsolidwalls, compute_uncovering_redistribute, final_uncovering_redistribute
...
contains
  ...
  subroutine cns_eb_compute_dudt (lo,hi, dudt, utlo, uthi, &
    ...
    real(rt), dimension(:,:,:), pointer, contiguous :: lambda, mu, xi
    integer, parameter :: nghost = 6 !nextra_eb + max(nghost_plm,3) ! 3 because of wall flux
    ...
    call compute_eb_divop(lo,hi,5,dx,dt,fhx,lfxlo,lfxhi,fhy,lfylo,lfyhi,fhz,lfzlo,lfzhi,&
	 ...
	 centroid, centlo, centhi, &
         ...
         levmsk, lmlo, lmhi, time,level, RKstep)
  ...
  subroutine zerooutsolidwalls(level, lo, hi, u, ulo, uhi, volfrac, vlo, vhi)bind(c,name='zerooutsolidwalls')

  implicit none

  integer, intent(in) :: level
  integer, dimension(3), intent(in) :: lo, hi, ulo, uhi, vlo, vhi
  real(rt), intent(inout) :: u ( ulo(1): uhi(1), ulo(2): uhi(2), ulo(3): uhi(3),14)
  real(rt), intent(in) :: volfrac(vlo(1):vhi(1),vlo(2):vhi(2),vlo(3):vhi(3))
  integer :: i, j, k, compno

  do k = lo(3), hi(3)
     do j = lo(2), hi(2)
        do i = lo(1), hi(1)
		if(volfrac(i,j,k).eq.0.0d0)then
		u(i,j,k,1) = 1.226d0
		u(i,j,k,2:14)=0.0d0
		endif
		do compno = 1, 14
		if(abs(u(i,j,k,compno)).le.1e-16) u(i,j,k,compno)=0.0d0
		enddo
	end do
    end do
  end do  

  end subroutine zerooutsolidwalls

  subroutine compute_uncovering_redistribute(lo, hi, volfracborder, volfracborderlo, volfracborderhi, &
				 volfrac, vlo, vhi, Sborder, Slo, Shi,& 
				 cons_adjust, cons_adjustlo, cons_adjusthi,&
			         donortag, donortaglo, donortaghi)bind(c,name='compute_uncovering_redistribute')

  implicit none
  integer, dimension(3), intent(in) :: lo, hi, volfracborderlo, volfracborderhi, vlo, vhi, Slo, Shi, cons_adjustlo, cons_adjusthi,donortaglo, donortaghi
  real(rt), intent(in) :: volfracborder ( volfracborderlo(1): volfracborderhi(1), volfracborderlo(2): volfracborderhi(2), volfracborderlo(3): volfracborderhi(3),1)
  real(rt), intent(in) :: volfrac(vlo(1):vhi(1),vlo(2):vhi(2),vlo(3):vhi(3))
  real(rt), intent(inout) :: Sborder(Slo(1):Shi(1),Slo(2):Shi(2),Slo(3):Shi(3),8)
  real(rt), intent(inout) :: cons_adjust(Slo(1):Shi(1),Slo(2):Shi(2),Slo(3):Shi(3),8)
  real(rt), intent(inout) :: donortag(Slo(1):Shi(1),Slo(2):Shi(2),Slo(3):Shi(3),1)
  integer :: i, j, k, ii, jj, kk, compno
  real(rt) :: sigma_kappa, volwgtavg
  ...
  do k = lo(3)-1, hi(3)+1
     do j = lo(2)-1, hi(2)+1
        do i = lo(1)-1, hi(1)+1
	   do compno = 1, 5
		if(abs(Sborder(i,j,k,compno)).le.1e-16)Sborder(i,j,k,compno)=0.0d0
	   end do

	   ! If this is an uncovered cell - i.e. the new volfrac > 0, but old volfrac=0, then give a tag of 1.
	   if(volfrac(i,j,k) .ne. 0.0d0 .and. volfracborder(i,j,k,1) .eq. 0.0d0)then
		!print*, "An uncovering event has happened", volfrac(i,j,k), volfracborder(i,j,k,1)	
		!print*, -1.0d0+(i+0.5d0)*2.0d0/64.0d0, -1.0d0+(j+0.5d0)*2.0d0/64.0d0, -0.125d0+(k+0.5d0)*0.25d0/8
		donortag(i,j,k,1) = 1.0d0
  	        sigma_kappa = 0.0d0
		do kk = -1, 1
		   do jj = -1, 1
		      do ii = -1, 1
			 ! Any cell in this neighborhood that was a valid fluid cell in the previous step (i.e. a cutcell or a regular cell,
			 ! which means old volfrac > 0) is an eligible donor. Give a tag of -1.
			 if(volfracborder(i+ii,j+jj,k+kk,1).ne.0.0d0)then
   			   donortag(i+ii,j+jj,k+kk,1) = -1.0d0
			   sigma_kappa = sigma_kappa + volfracborder(i+ii,j+jj,k+kk,1)
			 endif
		      end do
		   end do
		end do

	 	! Include the self as well
		sigma_kappa = sigma_kappa !+ volfrac(i,j,k)			
	      ...
	      do compno = 1, 7
                 ...
                 do kk = -1, 1
                    do jj = -1, 1
                       do ii = -1, 1
			 ! If this is a valid donor
			 if(volfracborder(i+ii,j+jj,k+kk,1).ne.0.0d0)then
                            volwgtavg = volwgtavg + volfracborder(i+ii,j+jj,k+kk,1)*Sborder(i+ii,j+jj,k+kk,compno)
                            cons_adjust(i+ii,j+jj,k+kk,compno) = cons_adjust(i+ii,j+jj,k+kk,compno) + &
                                                                 Sborder(i+ii,j+jj,k+kk,compno)*volfracborder(i+ii,j+jj,k+kk,1)*volfrac(i,j,k)/sigma_kappa
		
                           endif
                        end do
                     end do
                  end do
		! The new cell value is a volume weighted interpolated from the valid donors
                cons_adjust(i,j,k,compno) = volwgtavg/sigma_kappa
		if(compno.eq.5)then
		!print*, "Newly emerged", volwgtavg/sigma_kappa
		endif
                end do
  ...
  end subroutine compute_uncovering_redistribute

  subroutine final_uncovering_redistribute(lo, hi, volfracborder, volfracborderlo, volfracborderhi, &
					   volfrac, vlo, vhi, Sborder, Slo, Shi, &
	   				   cons_adjust, cons_adjustlo, cons_adjusthi,&
			                   donortag, donortaglo, donortaghi)bind(c,name='final_uncovering_redistribute')
  ...
  integer :: i, j, k, compno
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
	    ...
	    if(donortag(i,j,k,1).eq.-1.0d0)then
		!stop
      	     do compno = 1, 7
	   	Sborder(i,j,k,compno) =  Sborder(i,j,k,compno)! (Sborder(i,j,k,compno)*volfracborder(i,j,k,1) - cons_adjust(i,j,k,compno))/volfracborder(i,j,k,1)
		!Sborder(i,j,k,compno) = Sborder(i,j,k,compno)*volfracborder(i,j,k,1)/volfrac(i,j,k)
             end do

	    elseif(donortag(i,j,k,1).eq.1.0d0)then
		!stop
	    do compno = 1, 7
		Sborder(i,j,k,compno) = cons_adjust(i,j,k,compno)
	    end do
  ...
  end subroutine final_uncovering_redistribute
