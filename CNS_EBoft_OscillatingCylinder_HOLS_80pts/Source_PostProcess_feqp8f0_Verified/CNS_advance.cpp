...
//      dt=0.0;
    ...
    FillPatch(*this, Sborder, NUM_GROW, time, State_Type, 0, NUM_STATE);
    compute_dSdt(Sborder, dSdt, 0.5*dt, fr_as_crse, fr_as_fine, time, 1);
    // U^* = U^n + dt*dUdt^n
    ...
    compute_dSdt(Sborder, dSdt, 0.5*dt, fr_as_crse, fr_as_fine,time, 2);
...
CNS::compute_dSdt (const MultiFab& S, MultiFab& dSdt, Real dt,
                   EBFluxRegister* fr_as_crse, EBFluxRegister* fr_as_fine, Real time, int RKstep)
{
...
#pragma omp parallel
#endif
    {
                        ...
                        mfi.isValid(); ++mfi)
        {
			...
			const int finest_level = level==parent->finestLevel()	? parent->finestLevel() : -1;
