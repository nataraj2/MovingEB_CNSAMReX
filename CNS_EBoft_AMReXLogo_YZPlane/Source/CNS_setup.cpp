...
static Box my_grow_box (const Box& b) {
    return amrex::grow(b, 1);  // or whatever ngrow you want
}
...
CNS::variableSetUp ()
{
    ...
    cnt++; set_scalar_bc(bc,phys_bc); bcs[cnt] = bc; name[cnt] = "Volfraction";
	...
	derive_lst.add("vorticity",IndexType::TheCellType(),1,
                   cns_vorticity,my_grow_box);
	derive_lst.addComponent("vorticity",desc_lst,State_Type,Density,1);
    derive_lst.addComponent("vorticity",desc_lst,State_Type,Ymom,1);
    derive_lst.addComponent("vorticity",desc_lst,State_Type,Zmom,1);
