...
contains
  ...
  ! This routine will derive the velocity from the momentum.
  subroutine cns_vorticity(vort,v_lo,v_hi,nv, &
                       dat,d_lo,d_hi,nc,lo,hi,domlo, &
                       domhi,delta,xlo,time,dt,bc,level,grid_no) &
                       bind(C, name="cns_vorticity")
    implicit none
    ...
    integer, intent(in) :: bc(3,2,nc)
    real(rt), intent(in) :: delta(3), xlo(3), time, dt
    real(rt), intent(inout) :: vort(v_lo(1):v_hi(1),v_lo(2):v_hi(2),v_lo(3):v_hi(3))
    real(rt), intent(in   ) :: dat(d_lo(1):d_hi(1),d_lo(2):d_hi(2),d_lo(3):d_hi(3),nc)
    integer, intent(in) :: level, grid_no
	...
	!print*, lo(3), hi(3), d_lo(3), d_hi(3)

    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          do i = lo(1), hi(1)
			 if(dat(i,j-1,k,1) .ne. 0.0d0 .and. dat(i,j+1,k,1) .ne. 0.0d0 .and. &
			    dat(i,j,k+1,1) .ne. 0.0d0 .and. dat(i,j,k-1,1) .ne. 0.0d0)then

             	dvdz = (dat(i,j,k+1,2) / dat(i,j,k+1,1) - dat(i,j,k-1,2) / dat(i,j,k-1,1))/(2.0d0*delta(3))
             	dwdy = (dat(i,j+1,k,3) / dat(i,j+1,k,1) - dat(i,j-1,k,3) / dat(i,j-1,k,1))/(2.0d0*delta(2))
				vort(i,j,k) = abs(dvdz-dwdy)			  	
			 else
				vort(i,j,k) = 0.0d0
			 endif
          end do
