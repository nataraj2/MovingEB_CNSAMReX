...
contains
  ...
  subroutine cns_compute_vorticity (lo,hi,u,ulo,uhi,dx,vort,vortlo,vorthi) bind(c,name='cns_compute_vorticity')
    use cns_physics_module, only : cv
    integer, intent(in) :: lo(3), hi(3), ulo(3), uhi(3), vortlo(3), vorthi(3)
    real(rt), intent(inout) :: u(ulo(1):uhi(1),ulo(2):uhi(2),ulo(3):uhi(3),nvar)
    real(rt), intent(inout) :: vort(vortlo(1):vorthi(1),vortlo(2):vorthi(2),vortlo(3):vorthi(3),1)
    ...
    do       k = lo(3),hi(3)
       do    j = lo(2),hi(2)
          do i = lo(1),hi(1)
			 if(u(i,j,k+1,urho) .ne. 0.0d0 .and. u(i,j,k-1,urho) .ne. 0.0d0 .and. &
			    u(i,j+1,k,urho) .ne. 0.0d0 .and. u(i,j-1,k,urho) .ne. 0.0d0)then 
			 	dvdz = (u(i,j,k+1,umy)/u(i,j,k+1,urho) - u(i,j,k-1,umy)/u(i,j,k-1,urho))/(2.0*dx(2))
			 	dwdy = (u(i,j+1,k,umz)/u(i,j+1,k,urho) - u(i,j-1,k,umz)/u(i,j-1,k,urho))/(2.0*dx(1))
			 else
				dvdz = 0.0d0
			    dwdy = 0.0d0
			 endif
			 vort(i,j,k,1) = abs(dvdz-dwdy)
          end do
       end do
    end do
  end subroutine cns_compute_vorticity
