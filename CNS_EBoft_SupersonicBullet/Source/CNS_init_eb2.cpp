                ...
                const int max_coarsening_level,Real time)
{
   ...
   else if (geom_type == "supersonic_bullet")

   {
	...
	double shift;
        ...
        timestopaccel = 0.001;
        velstopaccel = accel*timestopaccel;
        shift = 680.32*time;//0.5*accel*time*time;
        //if(time > timestopaccel){
        //shift = 0.5*accel*timestopaccel*timestopaccel+velstopaccel*(time-timestopaccel);
       // }
	...
	EB2::CylinderIF cf(0.03175, 0.01913, 2, {0.0,0.0,0.01}, false);
	...
	bool has_fluid_inside=0;
	EB2::EllipsoidIF ellipsoid(radius, center, has_fluid_inside);
	...
	EB2::SplineIF TriForCone;
        ...
        pts.push_back(p);
        p = amrex::RealVect(AMREX_D_DECL(-0.0195665,0.04891,0.0));
        pts.push_back(p);
	...
	auto cone_translate = EB2::translate(Cone,{0.0,0.0,0.029});

	RealArray point;
        RealArray normal;

        point[0] = 0.0 ; point[1]=0.0  ; point[2] = 0.022;
        normal[0] = 0.0 ; normal[1]=0.0 ; normal[2]= 1.0;

        EB2::PlaneIF pf(point, normal);

	auto conefinal = EB2::makeIntersection(cone_translate,pf);
	auto polys = EB2::makeUnion(cf,ellipsoid,conefinal);
	//auto polys = EB2::makeUnion(Cone);

	auto polys_translate = EB2::translate(polys,{0.0,0.0,0.01+shift});

        auto gshop = EB2::makeShop(polys_translate);
        EB2::Build(gshop, geom, max_coarsening_level, max_coarsening_level, 4);
