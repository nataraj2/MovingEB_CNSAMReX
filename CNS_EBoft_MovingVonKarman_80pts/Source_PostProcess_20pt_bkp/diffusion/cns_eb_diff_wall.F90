...
contains
  ...
  subroutine compute_diff_wallflux (divw, dxinv, i,j,k, &
    ...
    un = q(i,j,k,qu)*anrmx + q(i,j,k,qv)*anrmy + q(i,j,k,qw)*anrmz

    ublade = 0.0d0!q(i,j,k,qu) 
    vblade = 0.0d0!q(i,j,k,qv)
    wblade = 50.0d0!q(i,j,k,qw)

    uwalldotn = (ublade*anrmx+vblade*anrmy+wblade*anrmz)

    uvel = ublade! - un*anrmx + uwalldotn*anrmx
    vvel = vblade! - un*anrmy + uwalldotn*anrmy
    wvel = wblade! - un*anrmz + uwalldotn*anrmz

    !uvel = ublade !- uwalldotn*anrmx
    !vvel = vblade !- uwalldotn*anrmy
    !wvel = wblade !- uwalldotn*anrmz
    ...
    dudn = -ddinv*(d2*d2*(u1-uvel)-d1*d1*(u2-uvel))  ! note that the normalvector points toward the wall
    ...
    if(abs(x-1.0d0/(dxinv(1)*2.0d0)).le.1e-5.and.level .eq. 2)then
